...
class FullyConnectedNet(object):
  ...
  def loss(self, X, y=None):
    ...
    for i in range(self.num_layers):
      ...
      if(i==0):
        if(self.use_batchnorm):
          h_i, cache = affine_batchnorm_relu_forward(X,self.params[w_i], self.params[b_i], self.params[gamma_i], self.params[beta_i],self.bn_params[i])
          if self.use_dropout:
            h_i, dro_cache = dropout_forward(h_i,self.dropout_param)
            dro_caches.append(dro_cache)
          hs.append(h_i)
          caches.append(cache)
        else:
        ...
        if(self.use_batchnorm):
          h_i, cache = affine_batchnorm_relu_forward(hs[i-1],self.params[w_i], self.params[b_i],self.params[gamma_i], self.params[beta_i],self.bn_params[i])
          if self.use_dropout:
            h_i, dro_cache = dropout_forward(h_i,self.dropout_param)
            dro_caches.append(dro_cache)
          hs.append(h_i)
    ...
    dLdhi,dLdwi, dLdbi, dLdgammai, dLdbetai =0,0,0,0,0
    for i in reversed(range(self.num_layers)):
      ...
      if(i == self.num_layers-1):
        ...
        if(self.use_batchnorm):
          if self.use_dropout:
            dLdupstream = dropout_backward(dLdupstream,dro_caches[i])
          dLdhi,dLdwi,dLdbi, dLdgammai, dLdbetai = affine_batchnorm_relu_backward(dLdupstream,caches[i])
          grads[gamma_i] = dLdgammai
          grads[beta_i] = dLdbetai
        else:
          if self.use_dropout:
            dLdupstream = dropout_backward(dLdupstream,dro_caches[i])
