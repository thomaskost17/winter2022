...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    sigma2_x = np.var(x,axis=0)
    running_mean = momentum * running_mean + (1 - momentum) * x_bar
    running_var  = momentum * running_var  + (1 - momentum) * sigma2_x

    #(2)
    x_hat = x - x_bar
    x_hat = x_hat/np.sqrt(sigma2_x+eps)
...
def batchnorm_backward(dout, cache):
  ...
  dgamma = np.ones(N,)@(dout*cache['xhat'])
  dxhat = dout*cache['gamma']
  xc = cache['x']-cache['xbar']
  da = 1/(np.sqrt(cache['sigma2']+cache['eps']))*dxhat
  dsigma2 = np.ones(N,)@((-0.5*1/np.power(cache['sigma2']+cache['eps'],1.5))*(xc)*dxhat)
  du =  -1/(np.sqrt(cache['sigma2']+cache['eps']))*(np.ones(N,)@dxhat) - dsigma2*(2/N**2)*(np.ones(N,)@(xc))
  dx = da +(2/N)*(cache['x'] - cache['xbar'])*dsigma2 +(1/N)*du
  # ================================================================ #
  # END YOUR CODE HERE
  # ================================================================ #
...
def dropout_backward(dout, cache):
  ...
  if mode == 'train':
    ...
    # We should not be doing backprop at test time so we can pass here
    pass
